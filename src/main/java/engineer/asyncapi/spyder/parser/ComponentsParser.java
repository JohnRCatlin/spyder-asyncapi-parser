		...
		try {
			...
			for (final String key : keys) {
				...
				if (isObjectNode(bindingNode)) {
					final ChannelBinding binding = ChannelBindingsParser.parse(bindingType, (ObjectNode) bindingNode);
					if (binding != null) {
	...
	private static final OperationBindings parseComponent(final ObjectNode node) {
		try {
			...
			for (final String key : keys) {
				...
				final JsonNode bindingNode = bindingTypeNode.get(bindingType);
				if (isObjectNode(bindingNode)) {
					final OperationBinding binding = OperationBindingParser.parse((ObjectNode) bindingNode, bindingType);
					if (binding != null) {
	...
	private static final ServerBindings parseServerBindings(final ObjectNode node) {
		try {
			...
			for (final String key : keys) {
				...
				final JsonNode bindingNode = bindingTypeNode.get(bindingType);
				if (isObjectNode(bindingNode)) {
					final ServerBinding binding = ServerBindingsParser.parse(bindingType, (ObjectNode) bindingNode);
					if (binding != null) {
						bindings.put(key, binding);
					}
				}
