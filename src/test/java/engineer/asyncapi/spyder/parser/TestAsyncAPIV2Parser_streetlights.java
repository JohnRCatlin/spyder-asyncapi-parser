...
public class TestAsyncAPIV2Parser_streetlights extends AsyncApiV2ParserTestBase {
	...
	private static final String SUBJECT_MODEL_SOURCE = "/src/test/resources/streetlights.yml";
	...
	public void channelsExpectations() {
		...
		assertTrue(
				api.getChannels().containsKey("smartylighting/streetlights/1/0/event/{streetlightId}/lighting/measured"));
		assertTrue(api.getChannels().containsKey("smartylighting/streetlights/1/0/action/{streetlightId}/turn/on"));
		assertTrue(api.getChannels().containsKey("smartylighting/streetlights/1/0/action/{streetlightId}/turn/off"));
		assertTrue(api.getChannels().containsKey("smartylighting/streetlights/1/0/action/{streetlightId}/dim"));

		/*
		 * first channel
		 */
		Channel channel1 = api.getChannels().get("smartylighting/streetlights/1/0/event/{streetlightId}/lighting/measured");
		assertNotNull(channel1);
		assertTrue(channel1.getDescription().contains("The topic on which measured values may be produce"));
		assertNotNull(channel1.getParameters());
		assertTrue(channel1.getParameters().containsKey("streetlightId"));
		assertEquals("#/components/parameters/streetlightId", channel1.getParameters().get("streetlightId").getRef());
		...
		assertTrue(operationTraitListContainsRef(channel1.getPublish().getTraits(), "#/components/operationTraits/kafka"));
		assertNotNull(channel1.getPublish().getMessage());
		assertEquals("#/components/messages/lightMeasured", channel1.getPublish().getMessage().getRef());
		 ...
		 * second channel
		 */
		Channel channel2 = api.getChannels().get("smartylighting/streetlights/1/0/action/{streetlightId}/turn/on");
		...
		assertEquals("#/components/parameters/streetlightId", channel2.getParameters().get("streetlightId").getRef());
		...
		assertTrue(
				operationTraitListContainsRef(channel2.getSubscribe().getTraits(), "#/components/operationTraits/kafka"));
		 ...
		 * third channel
		 */
		Channel channel3 = api.getChannels().get("smartylighting/streetlights/1/0/action/{streetlightId}/turn/off");
		...
		assertEquals("#/components/parameters/streetlightId", channel3.getParameters().get("streetlightId").getRef());
		...
		assertTrue(
				operationTraitListContainsRef(channel3.getSubscribe().getTraits(), "#/components/operationTraits/kafka"));
		 ...
		 * fourth channel
		 */
		Channel channel4 = api.getChannels().get("smartylighting/streetlights/1/0/action/{streetlightId}/dim");
		...
		assertEquals("#/components/parameters/streetlightId", channel4.getParameters().get("streetlightId").getRef());
		...
		assertTrue(
				operationTraitListContainsRef(channel4.getSubscribe().getTraits(), "#/components/operationTraits/kafka"));
	...
	public void componentsMessagesExpectations() {
		...
		assertTrue(
				message1.getSummary().contains("Inform about environmental lighting conditions of a particular streetlight."));
		assertEquals("application/json", message1.getContentType());
		assertNotNull(message1.getTraits());

		assertTrue(messageTraitListContainsRef(message1.getTraits(), "#/components/messageTraits/commonHeaders"));
		...
		assertTrue(message2.getSummary().contains("Command a particular streetlight to turn the lights on or off."));
		assertNotNull(message2.getTraits());

		assertTrue(messageTraitListContainsRef(message2.getTraits(), "#/components/messageTraits/commonHeaders"));
		...
		assertTrue(message3.getSummary().contains("Command a particular streetlight to dim the lights."));
		assertNotNull(message3.getTraits());

		assertTrue(messageTraitListContainsRef(message3.getTraits(), "#/components/messageTraits/commonHeaders"));
...
//		assertEquals(new BigDecimal(100), trait.getHeaders().getProperties().get("my-app-header").getMaximum());
	...
	public void componentsSchemasExpectations() {
		...
		assertTrue(schema1.getProperties().get("lumens").getDescription().contains("Light intensity measured in lumens."));
		schema1 = null;
		...
		// TODO is this a bug?
		// The jackson parser seems to convert the values on and of to true and false.
		// It interprets them correctly if we use 'on' and 'of' (see streetlights2.yml).
		...
		assertTrue(
				schema2.getProperties().get("command").getDescription().contains("Whether to turn on or off the light."));
		schema2 = null;
...
//		assertEquals(Type.OAUTH2, api.getComponents().getSecuritySchemes().get("supportedOauthFlows").getType());
		...
		OAuthFlow implicitFlow = api.getComponents().getSecuritySchemes().get("supportedOauthFlows").getFlows()
				.getImplicit();
		...
		OAuthFlow passwordFlow = api.getComponents().getSecuritySchemes().get("supportedOauthFlows").getFlows()
				.getPassword();
		...
		OAuthFlow clientCredentialsFlow = api.getComponents().getSecuritySchemes().get("supportedOauthFlows").getFlows()
		...
		OAuthFlow autorizationCodeFlow = api.getComponents().getSecuritySchemes().get("supportedOauthFlows").getFlows()
	...
	public void infoExpectations() {
		...
		assertTrue(api.getInfo().getDescription().contains("The Smartylighting Streetlights API allows you to remotely"));
